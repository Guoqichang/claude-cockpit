...
装好后打开 [http://127.0.0.1:7799/#about](http://127.0.0.1:7799/#about) 能看到项目介绍，和一轮会动的演示：你说 *把顶栏登录按钮改成金色* → 驾驶舱 7799 转给 OpenCode → OpenCode 改 `src/App.jsx` 里的 `btn-muted` → 磁盘上变成 `btn-gold`。关掉浏览器，改动还在。

## 这是什么（Vibecoding 入门）

场景钉死一件事：改本地项目顶栏登录按钮的颜色。以前自己翻 `src/App.jsx`。现在对着一条会话说人话，让带工具的模型去改那个文件。

驾驶舱不是又一个聊天网站。OpenCode / Claude / Cursor 才是引擎；驾驶舱只把本机已经存在的会话收进左侧列表。

先装 **1 个 OpenCode 二进制**、贴 **1 把 API Key**，端口 **7799** 就能聊。Claude 和 Cursor 是加餐，不是入场券。

| 词 | 从刚才那一轮推出来 |
|---|---|
| 引擎 | 真正读文件、改 `App.jsx` 的进程。驾驶舱自己不改代码。 |
| 会话 | 同一条工作线程，有工作目录和上一轮上下文。左侧列表里的一行。 |
| 一轮 | 你发一条，引擎干活到交付。关掉 7799 标签页只断画面。 |

五条够用的习惯：说要改哪别说气氛；钉死工作目录；一小步验一次；Key 是账单只留在本机 `~/.config/opencode/opencode.json`；飞轮从 OpenCode 开始。
...
个人 ASR 词典：每完成 8 轮聊天（`~/.claude-cockpit/config.json` 里 `asrLexicon.everyChats` 可改），驾驶舱会抽一次你的原话专名，写到 `~/.claude-cockpit/asr-lexicon.json`，Fun-ASR 试听台启动时读进来当热词。助手回复另走确认通道：用户没说过的词不进表。`GET /api/lexicon` 看状态。

输入框右下角麦克风走同一套 Fun-ASR：浏览器把 16k PCM 推到驾驶舱 `/ws/asr`，再转到试听台 `/ws/dictate`（不占听课）。停顿 0.6 秒出一段字，填进输入框，不自动发送。试听台要先在 8780 开着；听课中会拒口播。地址可在 `asrLexicon.url` 改。
...
⚠️ **拓扑排序必须容忍环**，这点一开始搞错了。blob 是内容寻址的：两条一模一样的消息（用户两次都回「好了」、同一条工具输出重复出现）会折成**同一个 blob id**。这个 id 于是在不同快照里占着不同位置，相邻关系互相矛盾 —— 长会话必然成环。最初的实现遇到环就整段返回 `null`，结果一个 2711 条的会话有 **2310 条卡在环里**，界面上显示「这个会话在 Cursor 侧是空的」，而数据其实完好无损。现在改成：无环约束照常生效，真卡住了就挑剩余入度最小的节点强制出队（它最接近「本该先出现」）。实测那个会话只需强制打破 **1 次**环就能把 2711 个节点全部排出，角色序列健康（游程平均 1.64，符合一次多个并行工具调用的形态）。

**怎么验证顺序对不对**（踩过一次判据陷阱）：不要用「工具结果和它的调用间距多大」——一条 assistant 消息可以并发发 17 个工具调用，最后一个结果的间距天然就是 17，间距大不等于乱序。正确判据是**把所有快照的相邻对拿出来数倒置**：现在的实现是 30/267320 = **0.01%**，最长那个 348 项快照零倒置。曾经试过两个「改进」都更差 —— 加 tool_call_id 配对边（倒置率不变，且解决不了真矛盾）、用跨快照平均位置做 tie-break（倒置率涨到 0.46%，差 40 倍，因为短快照的早期位置和长快照的晚期位置被混为一谈）。**tie-break 就用 blob id 字典序，别自作聪明。**

残留 1 处「工具结果排在调用之前」是内容寻址的固有代价：两个不同调用产生了一模一样的结果 → 折成同一个 blob → 「必须在 A 之后」和「必须在 B 之前」真矛盾，破环时只能牺牲一个。2693 条里 1 条，不值得为它加复杂度。

如果 store.db 里也确实没消息，会话正文会**照实报出 Cursor 自己写的死因**，而不是套模板。实测存在至少两种：额度耗尽（`You're out of usage`）和区域模型不可用（`Model not available ... not supported in your region`，换账号后选了本区不支持的模型）。后者和额度毫无关系，一律说成「额度耗尽」会误导人，所以 `stubError()` 把原始错误行取出来附在提示里。
...
等待最难受的不是慢，是分不清「在干活」还是「死了」。所以状态行下方那条波形**只由真实到达的事件驱动**：

- 有数据 → 起浪（正文/工具事件浪高，思考/增量浪低）；**没数据 → 迅速衰减成平线**，不粉饰
- 顶部一条发丝扫光只在本会话真的有轮次在跑时亮
- 静默到一定时长会说人话而不是让人干等：20s「多半在长考或等工具返回」→ 75s「仍在跑，不用刷新；关掉页面也不影响」→ 4min「卡死会自动收割并解锁，可以先去忙别的」

**为什么不做成常转的动画**：那正是 `运行中 16056s` 僵尸转圈骗过人的地方。动效必须是可信的证据，不能是安慰剂。
...
- **自动**：有轮次在跑时持一个 `caffeinate -d -i -m -s`（含防息屏），全部结束即释放
