...
## 会话图谱（`/api/graph` + `public/graph.js`）

把会话列表换成一张活地图：`◍ 会话图谱` 进入。

- **大小** = 轮数（log）× 新鲜度（72 小时内衰减），**颜色** = 聚类色系 + id 决定的明度，**内嵌小图标** 由 id 哈希决定，同色也能认人。
- **状态环**：绿色脉冲=正在跑、金色虚线转圈=goal、蓝色点线=loop、红环=上轮报错、淡绿虚线=别处已打开。运行中的节点**不用悬停**就显示 `▸ Bash` 这样的当前动作。
- **注意力优先**：按 attention 名次（正在跑 55 / 报错 45 / goal 30 / loop 20 / 新鲜度 25 / 体量 10）重排，最该看的落在正中。用**名次**而不是原始分定半径——分数扎堆在低位，直接用会挤成一个环。
- **聚类**：标题 + 目录名做 TF-IDF（中文按二元组），贪心聚类；单例按目录归并，超过 16 个的目录桶再按「今天/本周/本月/更早」拆开，否则一个大团吃掉整张图。标签会把重叠二元组缝成词（`告诉`+`诉我`→`告诉我`）。
- **布局**：向日葵（黄金角）铺点，大聚类靠中心。**别用环形锚点**——中间会空一大片。标签有避让，最多 26 个，冲突的让位给注意力高的。
- goal/loop **没有专用存储**，是从会话尾部 48KB 里扫最近的 `/goal`、`/loop` 命令还原的（`clear` 会取消）。
