...
import { searchContent as searchCodexContent } from './codex-sessions.js';
...
/* 只扫「transcript 被覆写成空壳」的那几个会话的 store.db，
   正常会话仍走文件树 grep，避免为了搜索去开几百个 sqlite。 */
function searchRecoveredCursor(q) {
  const needle = q.toLowerCase();
  const out = [];
  let slugs;
  try { slugs = fs.readdirSync(CURSOR_PROJECTS); } catch { return out; }
  for (const slug of slugs) {
    const dir = path.join(CURSOR_PROJECTS, slug, 'agent-transcripts');
    let ids;
    try { ids = fs.readdirSync(dir); } catch { continue; }
    for (const id of ids) {
      const fp = path.join(dir, id, id + '.jsonl');
      if (!fs.existsSync(fp) || !transcriptIsStub(fp)) continue;
      let msgs;
      try { msgs = readStoreMessages(id); } catch { continue; }
      if (!msgs) continue;
      let snippet = '';
      for (const m of msgs) {
        for (const b of m.blocks) {
          const t = b.type === 'text' ? b.text : (b.type === 'thinking' ? b.text : '');
          if (!t) continue;
          const at = t.toLowerCase().indexOf(needle);
          if (at === -1) continue;
          snippet = t.slice(Math.max(0, at - 40), at + 120).replace(/\s+/g, ' ').trim();
          break;
        }
        if (snippet) break;
      }
      if (!snippet) continue;
      const meta = findCursorSessionMeta(id);
      out.push({ engine: 'cursor', slug: meta?.slug || ('cursor:' + slug), id, snippet, where: 'content' });
    }
  }
  return out;
}
...
export async function searchSessions(rawQ, { engine = '' } = {}) {
  ...
  if (!want || want === 'cursor') {
    ...
    // 被额度错误覆写的 transcript 已经没有可 grep 的正文，
    // 内容只剩在 store.db 里；这类会话数量极少，单独补一遍不影响搜索耗时
    for (const h of searchRecoveredCursor(q)) add(h);
  }
  if (!want || want === 'hermes') {
    try {
  ...
  if (!want || want === 'codex') {
    try {
      const cx = searchCodexContent(q);
      for (const h of cx) add({ ...h, engine: 'codex', where: h.snippet ? 'content' : 'title' });
    } catch { /* Codex sessions may be unreadable */ }
  }
