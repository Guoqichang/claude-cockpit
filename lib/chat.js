...
import {
  codexBin, codexBinOk, cockpitZenBase, codexSpawnEnv, codexProviderFlags, unionAlphaModel,
} from './codex.js';
...
function shouldRetry(e, code) {
  if (e.engine === 'opencode' || e.engine === 'codex') return false;
...
function writeMeta(e) {
  try {
    fs.writeFileSync(metaPath(e.ch), JSON.stringify({
      ...
      // 轮次一句话都没产出就结束时，引擎那边没有 transcript，这是唯一还留着
      // "当时问了什么"的地方 —— 别让用户打的字凭空蒸发
      prompt: typeof e.prompt === 'string' ? e.prompt.slice(0, 2000) : undefined,
...
function drain(e) {
  ...
  for (const line of lines) {
    ...
    const events = e.engine === 'codex' ? mapCodexEvent(e, event) : [event];
    for (const ev of events) ingestEvent(e, ev);
  }
}

function ingestEvent(e, event) {
  ...
  if (event.type === 'assistant') {
    ...
    e.activity = { kind: 'text', name: String(event.event.delta.text).trim().slice(0, 60), at: Date.now() };
  } else if (event.type === 'connection' || event.type === 'retry') {
    if (event.subtype === 'reconnecting' || event.subtype === 'starting') {
      e.activity = { kind: 'tool', name: `重连中（第 ${event.attempt || 1} 次）`, at: Date.now() };
    } else if (event.subtype === 'reconnected') {
      e.activity = { kind: 'text', name: '连接已恢复', at: Date.now() };
    }
  } else if (event.type === 'tool_call') {
    const wrap = event.tool_call || {};
    const key = Object.keys(wrap).find(k => k.endsWith('ToolCall'));
    if (key) {
      const name = key.replace(/ToolCall$/, '').replace(/^./, c => c.toUpperCase());
      e.activity = event.subtype === 'completed'
        ? { kind: 'text', name: name + ' 完成', at: Date.now() }
        : { kind: 'tool', name, at: Date.now() };
    } else if (event.name) {
      e.activity = { kind: 'tool', name: event.name, at: Date.now() };
    }
  }
...
function mapCodexEvent(e, raw) {
  if (raw?.type === 'system' && !raw.payload) return [raw];
  const p = payloadOf(raw);
  const t = String(raw?.type || p.type || '');
  const sid = p.thread_id || p.session_id || p.id || raw.thread_id;
  if (sid && typeof sid === 'string' && sid.length > 8) e.sessionId = sid;

  if (t === 'thread.started' || t === 'session.created' || t === 'session_meta') {
    return [{ type: 'system', subtype: 'init', session_id: e.sessionId, model: e.model || 'union-alpha' }];
  }
  if (t === 'agent_message.delta' || t === 'message.delta' || (t === 'item.delta' && (p.delta?.text || p.text))) {
    const text = p.delta?.text || p.text || p.delta || '';
    if (!text) return [];
    e.fullText = (e.fullText || '') + text;
    return [{ type: 'stream_event', event: { type: 'content_block_delta', delta: { type: 'text_delta', text: String(text) } } }];
  }
  if (t === 'agent_message' || (t === 'item.completed' && (p.item?.type === 'agent_message' || p.type === 'agent_message'))) {
    const text = p.message || p.item?.text || p.text || e.fullText || '';
    if (!text) return [];
    return [{ type: 'assistant', session_id: e.sessionId, message: { role: 'assistant', content: [{ type: 'text', text: String(text) }] } }];
  }
  if (t === 'event_msg' && p.type === 'agent_message' && p.message) {
    return [{ type: 'assistant', session_id: e.sessionId, message: { role: 'assistant', content: [{ type: 'text', text: String(p.message) }] } }];
  }
  if (t === 'exec_command_begin' || (t === 'item.started' && /command|tool|exec/i.test(JSON.stringify(p.item || p)))) {
    const name = p.item?.command || p.command || p.item?.type || 'tool';
    return [{ type: 'tool_call', subtype: 'started', name: String(name).slice(0, 80), tool_call: { shellToolCall: { args: { command: name } } } }];
  }
  if (t === 'exec_command_end' || (t === 'item.completed' && /command|tool|exec/i.test(String(p.item?.type || '')))) {
    const name = p.item?.command || p.command || 'tool';
    return [{ type: 'tool_call', subtype: 'completed', name: String(name).slice(0, 80), tool_call: { shellToolCall: {} } }];
  }
  if (t === 'error' || t === 'turn.failed' || (t === 'event_msg' && p.type === 'error')) {
    const msg = p.message || p.error || JSON.stringify(p).slice(0, 300);
    return [{ type: 'result', subtype: 'error', is_error: true, result: String(msg), session_id: e.sessionId }];
  }
  if (t === 'turn.completed' || t === 'task_complete' || (t === 'event_msg' && p.type === 'task_complete')) {
    return [{
      type: 'result', subtype: 'success', is_error: false, session_id: e.sessionId,
...
function ocDirQs(cwd) {
  return 'directory=' + encodeURIComponent(cwd || os.homedir());
}

function ocPermissionRuleset(mode) {
  const action = mode === 'plan' ? 'ask' : 'allow';
  return [
...
function ocPermissionReply(mode) {
  if (mode === 'plan') return 'reject';
  if (mode === 'bypassPermissions') return 'always';
  return 'once';
}

async function ocReplyPermission(requestID, cwd, reply) {
  await ocFetch('/permission/' + encodeURIComponent(requestID) + '/reply?' + ocDirQs(cwd), {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
...
async function ocRejectQuestion(requestID, cwd) {
  await ocFetch('/question/' + encodeURIComponent(requestID) + '/reject?' + ocDirQs(cwd), {
    method: 'POST',
    timeoutMs: 8000,
  });
}
...
function startOpencodeChat(ch, opts) {
  ...
  e.spawnOpts = { ...e.spawnOpts, agent, engine: 'opencode', permissionMode };
  writeEvent(e, {
  ...
  (async () => {
    try {
      ...
      if (!sid) {
        const created = await ocFetch('/session?' + ocDirQs(cwd), {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            agent,
            ...
            permission: ocPermissionRuleset(permissionMode),
          }),
          timeoutMs: 15000,
          signal: abort.signal,
        });
      ...
      // OpenCode 按 directory 隔离事件流：不带这个参数只能收到 server.* 心跳，
      // 会话里的 message.part.delta 一个都到不了 —— 表现是轮次永远转圈（踩过）。
      const sse = await ocFetchRaw('/event?' + ocDirQs(cwd), { signal: abort.signal });
      if (!sse.ok || !sse.body) throw new Error('OpenCode 事件流连接失败');

      // OpenCode echoes the user prompt as message.part.updated. Track role
      // from message.updated so we never paint that echo as assistant text.
      const ocRoles = new Map(); // messageID -> role
      const promptText = String(opts.prompt || '').trim();
      const ocIsUserPart = (part) => {
        if (!part || typeof part !== 'object') return false;
        const mid = part.messageID || '';
        const role = (mid && ocRoles.get(mid)) || '';
        if (role === 'user') return true;
        if (role === 'assistant') return false;
        if (part.type === 'text' && promptText && String(part.text || '').trim() === promptText) {
          if (mid) ocRoles.set(mid, 'user');
          return true;
        }
        return false;
      };
      ...
      await ocFetch('/session/' + encodeURIComponent(sid) + '/prompt_async?' + ocDirQs(cwd), {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      for await (const ev of readSse(sse)) {
        ...
        if (type === 'permission.asked' || type === 'permission.v2.asked') {
          ...
          const reply = ocPermissionReply(permissionMode);
          writeEvent(e, {
            type: 'oc.permission',
            requestID: reqId,
          ...
          e.activity = {
            kind: 'tool',
            name: 'permission:' + (props.permission || props.action || 'ask'),
            at: Date.now(),
          };
          if (reqId) {
            try { await ocReplyPermission(reqId, cwd, reply); }
            catch (err) {
              writeEvent(e, {
                type: 'oc.permission',
                requestID: reqId,
          ...
          if (reqId) {
            try { await ocRejectQuestion(reqId, cwd); }
            catch { /* unanswered questions also hang the turn */ }
          }
        } else if (type === 'message.updated' && props.info) {
          const info = props.info;
          if (info.id && info.role) ocRoles.set(info.id, String(info.role).toLowerCase());
        } else if (type === 'message.part.updated' && props.part) {
          if (ocIsUserPart(props.part)) continue;
          ocNoteActivity(e, props.part);
          if (props.part.type === 'text' && props.part.text) {
            e.fullText = String(props.part.text);
          }
          writeEvent(e, { type: 'oc.part', part: props.part });
        } else if (type === 'message.part.delta' && props.delta) {
          const part = { id: props.partID, messageID: props.messageID, type: props.field === 'text' ? 'text' : '' };
          if (ocIsUserPart(part)) continue;
          writeEvent(e, {
            type: 'oc.delta',
...
// Cursor 的 API 是一组 AWS IP 轮询，本机直连只有部分能握手，表现为"时好时坏"
// （2026-09-02 实测 6 个 IP 只有 2 个能通，同一条命令跑 10 次成功 4 次）。
// 走代理端口时 CONNECT 会带上主机名，代理软件才能按域名规则选到可用线路 —— 实测 10/10。
// 只作用于 cursor 子进程，不动系统代理，也不影响 claude / opencode。
const CURSOR_PROXY = process.env.COCKPIT_CURSOR_PROXY || 'http://127.0.0.1:7890';

function withCursorProxy(baseEnv) {
  if (!CURSOR_PROXY || CURSOR_PROXY === 'off') return baseEnv;
  // 用户已经自己设过就尊重他的
  if (baseEnv.HTTPS_PROXY || baseEnv.https_proxy) return baseEnv;
  return {
    ...baseEnv,
    HTTPS_PROXY: CURSOR_PROXY,
    HTTP_PROXY: CURSOR_PROXY,
    NO_PROXY: [baseEnv.NO_PROXY, 'localhost,127.0.0.1,::1'].filter(Boolean).join(','),
  };
}

function startCodexChat(ch, opts) {
  if (!codexBinOk()) throw new Error('本机没有 Codex CLI（ChatGPT.app 里的 codex，或 PATH 上的 codex）');
  fs.mkdirSync(CHAT_DIR, { recursive: true });
  const cwd = opts.cwd || os.homedir();
  const model = unionAlphaModel(opts.model || 'union-alpha');
  const permissionMode = sanitizePermissionMode(opts.permissionMode);
  const prompt = String(opts.prompt || '').trim() || 'hi';
  const bin = codexBin();
  const args = ['exec', '--json', '--skip-git-repo-check', '-C', cwd];
  if (permissionMode === 'plan') args.push('-s', 'read-only');
  else args.push('--dangerously-bypass-approvals-and-sandbox');
  args.push(...codexProviderFlags(cockpitZenBase(), model));
  if (opts.resume) args.push('resume', opts.resume, prompt);
  ...
  const e = makeChatEntry(ch, { pid: child.pid, resume: opts.resume, cwd, model, engine: 'codex' });
  e.spawnOpts = { cwd, permissionMode, model, engine: 'codex' };
  e.prompt = prompt;
  writeEvent(e, { type: 'system', subtype: 'init', session_id: opts.resume || undefined, model });
  child.on('error', (err) => { emit(e, { kind: 'error', error: String(err) }); finish(e, -1); });
...
export function startChat(ch, opts) {
  ...
  if (opts.engine === 'codex') return startCodexChat(ch, opts);
  ...
  if (engine === 'cursor') {
    ...
    // Cursor 能靠 create-chat 预先拿到会话 id，Claude 不能 —— 它的 session_id 要等
    // 第一个事件才吐出来。这中间如果页面关掉，这个轮次就没有任何可寻址的身份：
    // 引擎还没落盘 transcript，前端 adoptLiveTurn 又要靠 sessionId 才能认领，
    // 结果就是"发完消息一关就再也找不到了"。改成由我们指定，spawn 那一刻身份就定了。
    if (!resume) {
  ...
  const e = {
    ch, pid: child.pid, sessionId: resume || newSessionId || null, cwd: cwd || os.homedir(),
...
/* 引擎要等一条消息完整才写进自己的 transcript。刷新时前端从会话文件读历史，
   缺的恰恰是"正在生成的这条"——而 from='future' 又把缓冲区整个跳过，
   于是那段正文既不在文件里也不会被回放，直接消失。
   改为从最后一个"已落盘的产物"之后开始回放：补上的正好是文件里没有的部分，
   既不丢字也不重复。 */
function lastPersistedBoundary(e) {
  for (let i = e.buffer.length - 1; i >= 0; i--) {
    const m = e.buffer[i];
    if (m.kind !== 'event') continue;
    const ev = m.event || {};
    if (ev.type === 'assistant' || ev.type === 'user' || ev.type === 'result') return i + 1;
    // cursor 的工具调用是独立事件，完成时也已经落进它自己的 transcript
    if (ev.type === 'tool_call' && ev.subtype === 'completed') return i + 1;
  }
  return 0;
}
...
export function subscribe(ch, from, fn) {
  ...
  if (!e) return null;
  const start = from === 'future'
    ? lastPersistedBoundary(e)
    : Math.max(0, from | 0);
...
export function listChats() {
  return [...chats.values()].map(e => ({
    ...
    // 引擎不会把用户的输入回显成事件，Cursor 也要等真正开始处理才写进自己的
    // transcript。这中间刷新页面，那句话既不在会话文件里、也不在事件流里，
    // 只有这份 meta 还留着 —— 暴露出来，重连时前端才能把它画回去。
