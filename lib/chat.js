...
// WS clients must not be able to ask for bypassPermissions (full tool access).
...
export function startChat(ch, opts) {
  const { cwd, resume, prompt, model, attachments, provider } = opts;
...
export function restoreChats() {
  ...
  for (const f of files) {
    ...
    const e = {
      ...
      model: meta.model || null, startedAt: meta.startedAt || Date.now(),
      buffer: [], subs: new Set(), offset: 0, rest: '', done: null, timer: null, exitCode: null,
    };
    chats.set(e.ch, e);
...
export function listChats() {
  return [...chats.values()].map(e => ({
    ch: e.ch, sessionId: e.sessionId, cwd: e.cwd, model: e.model,
