...
export function readStoreMessages(chatId) {
  ...
  try {
    ...
    /* Kahn 拓扑排序，但必须容忍环。
       blob 是内容寻址的：两条一模一样的消息（比如用户两次都回"好了"、
       同一条工具输出重复出现）会折成同一个 blob id。这个 id 于是在不同的
       上下文快照里占着不同位置，相邻关系互相矛盾 —— 长会话必然成环。
       原来遇到环就整段返回 null，实测一个 2711 条的会话有 2310 条卡在环里，
       界面上就变成"这个会话在 Cursor 侧是空的"，而数据其实完好。
       改成：无环约束照常生效；真的卡住了就挑剩余入度最小的节点强制出队
       （它最接近"本该先出现"），继续往下排。就绪集合按 id 排序，
       保证同一份库每次重建出同样的顺序。 */
    const order = [];
    const placed = new Set();
    const pending = new Set(seen);
    let ready = [...seen].filter(id => indeg.get(id) === 0).sort();
    while (pending.size) {
      while (ready.length && placed.has(ready[0])) ready.shift();
      if (!ready.length) {
        let best = null, bestDeg = Infinity;
        for (const id of pending) {
          const d = indeg.get(id);
          if (d < bestDeg || (d === bestDeg && (best === null || id < best))) { bestDeg = d; best = id; }
        }
        if (best === null) break;
        indeg.set(best, 0);
        ready.push(best);
      ...
      for (const m of adj.get(n)) {
        ...
        indeg.set(m, Math.max(0, indeg.get(m) - 1));
...
/* 空壳 transcript 里那一行错误就是 Cursor 自己写的死因。
   以前一律按"额度耗尽"解释，但实测还有别的写法（比如换了账号后
   "Model not available ... not supported in your region"），
   照着模板讲会误导人。把原文取出来，让调用方照实说。 */
export function stubError(fp) {
  try {
