...
const BUILTIN = [
  {
    id: 'union-alpha',
    label: 'Union Alpha Free（OpenCode Zen）',
    // Claude Code 走驾驶舱本地网关：Zen 免费档拒绝第三方 UA，网关再转 OpenCode session
    baseUrl: 'http://127.0.0.1:7799/zen-anthropic',
    model: 'union-alpha',
...
export function listProviders({ withKeys = false } = {}) {
  ...
  return providers.map(p => {
    ...
    if (p.id === 'union-alpha') {
      p = { ...p, baseUrl: `http://127.0.0.1:${port}/zen-anthropic` };
    }
    const hasKey = !!p.apiKey || (p.id === 'union-alpha' && !!opencodeZenKey());
    return withKeys ? p : { ...p, apiKey: undefined, hasKey };
  });
}
...
export function providerEnv(id) {
  ...
  if (!p) return null;
  const key = p.apiKey || process.env[p.keyRef] || (p.id === 'union-alpha' ? opencodeZenKey() : '') || '';
