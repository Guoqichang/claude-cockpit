...
import {
  listProjects as listOpencodeProjects,
  readSession as readOpencodeSession,
  findSessionMeta as findOpencodeSessionMeta,
  isOpencodeSlug,
  isOpencodeId,
} from './opencode-sessions.js';
import {
  listProjects as listCodexProjects,
  readSession as readCodexSession,
  findSessionMeta as findCodexSessionMeta,
  isCodexSlug,
  isCodexId,
...
export async function listProjects() {
  ...
  let opencode = [];
  try { opencode = listOpencodeProjects(); }
  catch { opencode = []; }
  let codex = [];
  try { codex = listCodexProjects(); }
  catch { codex = []; }
  const all = [...listClaudeProjects(), ...listCursorProjects(), ...hermes, ...opencode, ...codex];
...
export async function readSession(slug, id, opts) {
  const data = isOpencodeSlug(slug)
    ? readOpencodeSession(slug, id, opts)
    : isCodexSlug(slug)
      ? readCodexSession(slug, id, opts)
      : isHermesSlug(slug)
...
export async function findSessionMeta(id) {
  const oc = isOpencodeId(id) ? findOpencodeSessionMeta(id) : null;
  const cx = !oc && isCodexId(id) ? findCodexSessionMeta(id) : null;
  const meta = oc
    || cx
    || findClaudeSessionMeta(id)
