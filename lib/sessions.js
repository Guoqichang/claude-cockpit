...
function extractMeta(filePath) {
  ...
  if (stat.size > META_FULL_LIMIT) {
    ...
    const meta = { title: title || firstText.slice(0, 60) || '(长会话)', cwd, mtimeMs: stat.mtimeMs, msgCount: null };
    metaCache.set(filePath, { mtimeMs: stat.mtimeMs, meta });
    return meta;
  }
  const lines = fs.readFileSync(filePath, 'utf8').split('\n');
  let title = '', cwd = '', msgCount = 0, firstText = '';
  ...
  if (!title) title = firstText.slice(0, 60) || '(空会话)';
  ...
  if (typeof raw === 'string') {
    ...
    for (const b of raw) {
      if (b.type === 'text') {
        ...
        // history payloads are already heavy; skip inlining oversized images
...
export function readSession(slug, id, { end, limit = 200 } = {}) {
  ...
  return { id, slug, cwd, title, total, start: from, messages, bytes: stat.size };
}
