...
export const MEDIA_ROOT = path.join(os.homedir(), '.claude-cockpit', 'media');
const CURSOR_PROJECTS = path.join(os.homedir(), '.cursor', 'projects');
const HOME = os.homedir();
const TMP_ROOTS = ['/tmp', '/private/tmp', '/var/folders'];
const IMAGE_EXT = /\.(png|jpe?g|gif|webp|bmp|heic|heif|svg|avif)$/i;
const SECRET_DIR = /(^|[\\/])(\.ssh|\.gnupg|\.aws|\.kube|\.config\/gh)([\\/]|$)/;
...
export function isIndexedFilePath(fp) {
  ...
  return underRoot(real, UPLOAD_ROOT) || underRoot(real, CURSOR_PROJECTS) || underRoot(real, MEDIA_ROOT);
}

/** Images Claude embeds as markdown `![](/Users/…/foo.png)` — serve those too. */
export function isServableMediaPath(fp) {
  if (isIndexedFilePath(fp)) return true;
  if (!fp || typeof fp !== 'string' || fp.includes('\0')) return false;
  const real = realOrNull(fp);
  if (!real || SECRET_DIR.test(real) || !IMAGE_EXT.test(real)) return false;
  if (underRoot(real, HOME)) return true;
  return TMP_ROOTS.some((root) => real === root || real.startsWith(root + path.sep));
}

/** Spill a base64 image to disk so the browser loads a URL instead of a 4MB data: URI. */
export function persistImage(mediaType, data) {
  if (!data) return null;
  let buf;
  try { buf = Buffer.from(data, 'base64'); } catch { return null; }
  if (!buf.length || buf.length > 25 * 1024 * 1024) return null;
  fs.mkdirSync(MEDIA_ROOT, { recursive: true, mode: 0o700 });
  const ext = EXT_BY_MIME[mediaType] || '.png';
  const dest = path.join(MEDIA_ROOT, crypto.createHash('sha1').update(buf).digest('hex').slice(0, 16) + ext);
  if (!fs.existsSync(dest)) fs.writeFileSync(dest, buf, { mode: 0o600 });
  return '/api/local-file?path=' + encodeURIComponent(dest);
}
