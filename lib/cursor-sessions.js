...
function normalizeEntry(e) {
  ...
  if (typeof raw === 'string') {
    const t = role === 'user' ? stripReminders(raw) : raw;
    if (t) blocks.push({ type: 'text', text: t });
  } else if (Array.isArray(raw)) {
    for (const b of raw) {
      if (b.type === 'text') {
        const t = role === 'user' ? stripReminders(b.text || '') : (b.text || '');
        if (t) blocks.push({ type: 'text', text: t });
...
export function findSessionMeta(id) {
  ...
  for (const wsHash of wsDirs) {
    ...
    try {
      ...
      return { slug: SLUG_PREFIX + wsHash, engine: 'cursor', title: id.slice(0, 8), cwd: '' };
    }
  }
  return null;
}
  ...
  for (const wsHash of fs.readdirSync(CHATS_DIR)) {
    ...
    for (const chatId of chatIds) {
      ...
      sessions.push({
        id: chatId,
        engine: 'cursor',
        title: meta.title || chatId.slice(0, 8),
        cwd: meta.cwd || '',
