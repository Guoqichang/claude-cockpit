...
import { readStoreMessages, transcriptIsStub } from './cursor-store.js';
...
export function readSession(slug, id, { end, limit = 200 } = {}) {
  ...
  if (!fp || transcriptIsStub(fp)) {
    ...
    // transcript 被覆写、store 里也确实没内容：说清是 Cursor 那边的事，不是这里弄丢的
    return fromStore(slug, id, [{
      role: 'assistant',
      blocks: [{ type: 'text', text: '这个会话在 Cursor 侧是空的。\n\nCursor 账号额度耗尽时会把整个 transcript 文件覆写成一行错误，历史对话随之丢失；它自己的 store.db 里也没留下消息，说明这一轮从没成功跑起来过。\n\n（Cockpit 只读这两处，没有删除任何数据。）' }],
      timestamp: null,
