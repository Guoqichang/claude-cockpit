...
import { readStoreMessages, transcriptIsStub, stubError } from './cursor-store.js';
...
function autoTitle(chatId) {
  ...
  if (!fp || transcriptIsStub(fp)) {
    const rec = readStoreMessages(chatId);
    if (!rec) return '';         // 空壳且 store 也读不出来：下次轮询再试，不缓存
    let soft = '';
    for (const m of rec) {
      if (m.role !== 'user') continue;
      const t = m.blocks.find(b => b.type === 'text')?.text || '';
      if (!t) continue;
      const { title, quality } = pickTitleFromText(t);
      if (quality === 'hard' && title) { titleCache.set(chatId, title); return title; }
      if (!soft && title) soft = title;
    }
    if (soft) titleCache.set(chatId, soft);
    return soft;
  }
  let head = '';
  try {
...
/* 从 store.db 重建的会话：结构对齐 readSession 的返回值，
   多带一个 recovered 标记，让前端能说清这是从 Cursor 自己的库里捞回来的。 */
function fromStore(slug, id, messages, { end, limit, recovered }) {
  const wsHash = slug.slice(SLUG_PREFIX.length);
  let cwd = '', title = '';
  try {
    const meta = JSON.parse(fs.readFileSync(path.join(CHATS_DIR, wsHash, id, 'meta.json'), 'utf8'));
    cwd = meta.cwd || '';
    title = meta.title || '';
  } catch { /* optional */ }
  ...
  return {
    id, slug, engine: 'cursor', cwd,
    title: title || autoTitle(id) || '(无标题)',
    total, start: from, messages: messages.slice(from, to), bytes: 0,
...
export function readSession(slug, id, { end, limit = 200 } = {}) {
  ...
  if (!fp || transcriptIsStub(fp)) {
    const rec = readStoreMessages(id);
    if (rec) return fromStore(slug, id, rec, { end, limit, recovered: !!fp });
    if (!fp) throw new Error('cursor 会话 transcript 尚未落盘，请先在 Cursor 里说过至少一轮');
    // transcript 被覆写、store 里也确实没内容：照实说是 Cursor 那边的事，不是这里弄丢的
    const err = stubError(fp);
    const why = /out of usage|increase your limit/i.test(err)
      ? 'Cursor 账号额度耗尽，它把整个 transcript 覆写成了这一行错误。'
      : /not supported in your region|model not available/i.test(err)
        ? '这一轮选用的模型在当前区域不可用，Cursor 直接以错误收尾，没有产生任何对话。'
        : 'Cursor 以一行错误结束了这一轮，并把 transcript 覆写成了它。';
    const lines = [
      '这个会话在 Cursor 侧是空的。',
      '',
      why,
      '它自己的 store.db 里也没有留下消息，说明这一轮从没成功跑起来过。',
    ];
    if (err) lines.push('', 'Cursor 原始错误：', '> ' + err);
    lines.push('', '（Cockpit 只读 transcript 和 store.db 两处，没有删除任何数据。）');
    return fromStore(slug, id, [{
      role: 'assistant',
      blocks: [{ type: 'text', text: lines.join('\n') }],
      timestamp: null,
    }], { end, limit, recovered: false });
  }
  const stat = fs.statSync(fp);
