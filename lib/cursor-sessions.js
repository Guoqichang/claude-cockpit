...
function autoTitle(chatId) {
  ...
  for (const line of head.split('\n')) {
    ...
    if (!e) continue;
    const role = e.type === 'user' ? 'user' : e.role;
    ...
    const t = first.replace(/\s+/g, ' ').slice(0, 60);
    if (!t) continue;
    if (!/^(continue from where you left off|上一轮回复因连接中断|继续[。！!]?$)/i.test(t)) {
      titleCache.set(chatId, t);
      return t;
    }
    if (!fallback) fallback = t;
    if (++seen >= 6) break;       // resumed sessions: give up after a few turns
  }
  if (fallback) titleCache.set(chatId, fallback);
  return fallback;
}
